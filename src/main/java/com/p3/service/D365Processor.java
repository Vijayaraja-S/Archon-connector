...
public class D365Processor {
    ...
    private void processXmlMetadata(XMLMetadata xmlMetadata) {
        ...
        // Potential further processing can happen here.
    }


    private BufferedReader getBufferedReader(String authorityUrl, String parameters) throws IOException {
    ...
    private XMLMetadata analyzeMetadata(String accessToken, String metadataUrl) {
        try {
            ...
            XMLMetadataRequest xmlMetadataRequest = client
    ...
    private String getAccessToken(D365ConnectionInfo info) throws IOException {
        ...
        String authorityUrl = String.format("https://login.microsoftonline.com/%s/oauth2/token", info.getTenantId());
    ...
    private String buildUrlParameters(D365ConnectionInfo info) {
        return "resource=" + URLEncoder.encode(info.getResourceUrl(), StandardCharsets.UTF_8) +
                "&client_id=" + URLEncoder.encode(info.getClientId(), StandardCharsets.UTF_8) +
                "&client_secret=" + URLEncoder.encode(info.getClientSecret(), StandardCharsets.UTF_8) +
                "&grant_type=" + URLEncoder.encode(info.getGrantType(), StandardCharsets.UTF_8);
    }
